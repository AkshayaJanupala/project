...
public class UserDto {
	...
	public UserDto(@Positive(message = "{validation.user.id.positive}") Long userId,
			...
			@NotEmpty(message = "{validation.user.roles.required}") @Size(min = 1, max = 2, message = "{validation.user.roles.size}") Set<@Pattern(regexp = "^(?i)(ADMIN|CUSTOMER)$", message = "{validation.user.roles.invalid}") String> roles) {
		super();
		this.userId = userId;
